...
const history = useHistory();
  let isMail = true;
  let path = "/home";
console.log('helllooo top', empId)
    let rep = empId ? true : false 
    let val = !empId ? true : false 
    // let val = false
let v = {}
...
console.log(a, 'new employee aauth')
  ...
  useEffect( () => {
    ...
    const viewStatus = async (event) => {
        // setIsView(true);
        let rep = true
        // val = true
        console.log('data fetching', empId)
      ...
      const loadedEmpData = [];
            ...
            setElpData(responseData)
            loadedEmpData.push(responseData)

            // for (const key in responseData) {
            //     loadedEmpData.push({
            //        key: responseData[key],
            //     //    des : responseData[key]
            //     })}
      ...
      if(empId ) {
        console.log('data fetchiing',empId)
        viewStatus()
        // setIsLoading(false);
        console.log(elpData)
        // return (
      ...
      <form className="empData-form" onSubmit={empDataCheck}>
        <div className="pic">
          <figure className="newEmp-fig">
            <img src={empId ? (elpData.gen === 'male' ? maleURL : femaleURL) : (isSubscribed ? maleURL : femaleURL)} alt="Profile"></img>
          </figure>
          <div className="gen">
            {/* <p>{empId}</p> */}
            <input
              ...
              checked={empId ? (elpData.gen === 'male' ? true : false) : isSubscribed}
              id="Male"
              name="Male"
            ></input>
            ...
            <input
              ...
              id="Female"
              name="Female"
              checked={empId ? (elpData.gen === 'female' ? true : false) : !isSubscribed}
              value={!isSubscribed}
              onChange={handleChange}
            ></input>
        ...
        <div className="emp-lab">
          <label htmlFor="name"><CgProfile/></label>
          <input type="text" placeholder="Name" id="name" minLength="4"  value={empId ? (elpData.name ? elpData.name : '' ) : name }></input>
        </div>
        <div className="emp-lab">
          <label htmlFor="Email"><MdEmail/></label>
          <input type="email" placeholder="Email" id="Email" minLength="4" value={empId ? (elpData.email ? elpData.email : '' ) : email}></input>
            
          <label htmlFor="MoNo"><MdPhone/></label>
          <input type="number" placeholder='Mobile Number' id="MoNo" minLength="10" value={empId ? (elpData.no ? elpData.no : '' ) : no}></input>
        </div>
        <div className="emp-lab">
          <label htmlFor="dep"><FaBuilding/></label>
          <input type="text" id="dep" placeholder="Department" minLength="4" value={empId ? (elpData.dep ? elpData.dep : '' ) : dep}></input>
          <label htmlFor="des"><FaGraduationCap/></label>
          <input type="text" placeholder="Designation" id="des" minLength="4" value={empId ? (elpData.des ? elpData.des : '' ) : des}></input>
        </div>
        <div className="btn">
